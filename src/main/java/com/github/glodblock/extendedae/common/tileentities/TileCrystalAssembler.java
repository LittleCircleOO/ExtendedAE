...
public class TileCrystalAssembler extends AENetworkPowerBlockEntity implements IGridTickable, IUpgradeableObject, IConfigurableObject {
    ...
    private final GenericStackInv tank = new GenericStackInv(this::onChangeTank, GenericStackInv.Mode.STORAGE, 1);
    ...
    public TileCrystalAssembler(BlockPos pos, BlockState blockState) {
        super(FCUtil.getTileType(TileCrystalAssembler.class, TileCrystalAssembler::new, EAEItemAndBlock.CRYSTAL_ASSEMBLER), pos, blockState);
        this.getMainNode()
                .setFlags()
                .setIdlePowerUsage(0)
    ...
    public TickingRequest getTickingRequest(IGridNode node) {
        return new TickingRequest(TickRates.Inscriber, !this.ctx.shouldTick(), false);
    }
    ...
    private static class CrystalRecipeContext extends RecipeSearchContext<CrystalAssemblerRecipe> {
        ...
        public void runRecipe(RecipeHolder<CrystalAssemblerRecipe> recipe) {
            ...
            if (fluid != null && fluid.what() instanceof AEFluidKey key) {
                fluidStack = new FluidStack(key.toVariant(), fluid.amount());
            }
            for (var tester : sample) {
            ...
            if (fluidStack != null) {
                if (fluidStack.isEmpty()) {
                    ...
                    this.host.tank.setStack(0, new GenericStack(AEFluidKey.of(fluidStack.getType()), fluidStack.getAmount()));
                }
            }
        }
        ...
        public List<Object> copyHostInv() {
            ...
            var fluid = this.host.tank.getStack(0);
            if (fluid != null && fluid.what() instanceof AEFluidKey key) {
                inv.add(new FluidStack(key.toVariant(), fluid.amount()));
            }
            return inv;
        }
