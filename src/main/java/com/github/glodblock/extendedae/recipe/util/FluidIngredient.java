...
public class FluidIngredient implements Predicate<FluidStack> {
    ...
    public static FluidIngredient of(FriendlyByteBuf buff) {
        ...
        if (type == 0) {
            return new FluidIngredient(new FluidValue(FluidStackTool.readFluidStack(buff)));
        } else if (type == 1) {
    ...
    public void to(FriendlyByteBuf buff) {
        if (this.value instanceof FluidValue f) {
            buff.writeByte(0);
            FluidStackTool.writeFluidStack(buff, f.fluid);
...
class FluidStackTool{
    static void writeFluidStack(FriendlyByteBuf buff, FluidStack stack) {
        if (stack.isEmpty()) {
            buff.writeBoolean(false);
        } else {
            buff.writeBoolean(true);
            stack.writeToPacket(buff);
        }

    }

    static FluidStack readFluidStack(FriendlyByteBuf buff) {
        return !buff.readBoolean() ? FluidStack.EMPTY : FluidStack.readFromPacket(buff);
    }
}